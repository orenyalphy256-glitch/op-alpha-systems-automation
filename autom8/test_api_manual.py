...
def main():
    ...
    print("\n[TEST 3] Create New Contact")
    new_contact = {"name": "API Test User", "phone": "0798888888", "email": "apitest@example.com"}
    response = requests.post(f"{BASE_URL}/contacts", json=new_contact, timeout=TIMEOUT)
    ...
    print("\n[TEST 4] Get Single Contact")
    response = requests.get(f"{BASE_URL}/contacts/{contact_id}", timeout=TIMEOUT)
    ...
    response = requests.put(f"{BASE_URL}/contacts/{contact_id}", json=update_data, timeout=TIMEOUT)
    print_response(response, f"PUT /contacts/{contact_id}")
    sleep(1)

    # Test 6: Search contacts
    print("\n[TEST 6] Search Contacts")
    response = requests.get(f"{BASE_URL}/contacts?search=API", timeout=TIMEOUT)
    print_response(response, "GET /contacts?search=API")
    sleep(1)

    # Test 7: Delete contact
    print("\n[TEST 7] Delete Contact")
    response = requests.delete(f"{BASE_URL}/contacts/{contact_id}", timeout=TIMEOUT)
    print(f"\n{'='*60}")
    ...
    response = requests.post(f"{BASE_URL}/contacts", json=duplicate, timeout=TIMEOUT)
    ...
    response = requests.post(f"{BASE_URL}/contacts", json=invalid, timeout=TIMEOUT)
