...
class TestDatabaseTransactions:
    ...
    def test_transaction_isolation(self, test_db):
        ...
        # Act
        committed_contacts = test_db.query(Contact).filter(Contact.phone.in_(["0700000001", "0700000002"])).all()
...
class TestBulkOperations:
    ...
    def test_bulk_insert(self, test_db):
        ...
        # Arrange
        contacts = [
            Contact(name=f"User {i}", phone=f"070000000{i}")
            for i in range(100)
        ]
    ...
    def test_bulk_update(self, test_db_with_data):
        ...
        # Act
        test_db_with_data.query(Contact).update(
            {"name": "Updated Name"},
...
class TestQueryPerformance:
    ...
    def test_query_with_filter(self, test_db_with_data):
        ...
        # Act
        alice = test_db_with_data.query(Contact).filter(
    ...
    def test_query_with_like(self, test_db_with_data):
        """Test LIKE queries."""
        # Act
        results = test_db_with_data.query(Contact).filter(
            Contact.name.like("%Smith%")
        ).all()
    ...
    def test_query_ordering(self, test_db_with_data):
        ...
        # Act
        contacts_asc = test_db_with_data.query(Contact).order_by(
            Contact.name.asc()
        ).all()
        
        contacts_desc = test_db_with_data.query(Contact).order_by(
            Contact.name.desc()
        ).all()
