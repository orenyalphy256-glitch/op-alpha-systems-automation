...
from autom8 import db_shell, init_database, seed_data, inspect_db
...
def test_inspect_db():
    ...
    with patch("autom8.inspect_db.get_session", return_value=mock_session):
        with patch("sys.stdout", new=StringIO()):
            ...
            with patch("autom8.inspect_db.engine") as mock_eng:
                inspect_db.main()
...
def test_db_shell_main_session(capsys):
    ...
    mock_session.query.return_value.limit.return_value.offset.return_value.all.return_value = [mock_contact]
